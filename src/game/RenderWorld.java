...
public class RenderWorld {
    ...
    public RenderWorld(World world, TextureLoader tl){
        this.world = world;
        this.tl = tl;
    }
        ...
        for(int i = 0; i < 16; i ++){
            for(int j = 0; j < 256; j ++){
                for(int k = 0; k < 16; k ++){
                    ...
                    if(!block.isOpaque()){
                        BlockRender.initAllFaces(i + xOffset, j, k + zOffset, 1, 1, 1, pos.vbo, tl, block.textureId);
                        continue;
                    }
                    ...
                    if(l == null || !l.isOpaque()){
                        BlockRender.initLeft(i + xOffset, j, k + zOffset, 1, 1, 1, pos.vbo, tl, block.textureId[4]);
                    }
                    if(r == null || !r.isOpaque()){
                        BlockRender.initRight(i + xOffset, j, k + zOffset, 1, 1, 1, pos.vbo, tl, block.textureId[2]);
                    }
                    if(d == null || !d.isOpaque()){
                        BlockRender.initDown(i + xOffset, j, k + zOffset, 1, 1, 1, pos.vbo, tl, block.textureId[5]);
                    }
                    if(u == null || !u.isOpaque()){
                        BlockRender.initUp(i + xOffset, j, k + zOffset, 1, 1, 1, pos.vbo, tl, block.textureId[0]);
                    }
                    if(f == null || !f.isOpaque()){
                        BlockRender.initFront(i + xOffset, j, k + zOffset, 1, 1, 1, pos.vbo, tl, block.textureId[1]);
                    }
                    if(b == null || !b.isOpaque()){
                        BlockRender.initBack(i + xOffset, j, k + zOffset, 1, 1, 1, pos.vbo, tl, block.textureId[3]);
                    }
                }
            }
        }
    ...
    public void render(){
        ...
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, tl.getTextureId());
